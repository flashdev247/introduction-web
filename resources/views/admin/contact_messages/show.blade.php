...
@section('header_title', 'View Message')
@section('content')
<div class="page-header">
    <h1><i class="fas fa-envelope-open-text"></i> Message Details</h1>
...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px; border-bottom: 1px solid #edf2f7; padding-bottom: 20px;">
        <div>
            ...
            <p style="color: #718096; font-size: 14px;">
                <i class="far fa-calendar-alt"></i> Received on {{ $message->created_at->format('M d, Y \a\t H:i') }}
            </p>
        </div>
        <div>
            @if($message->is_read)
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 24px; margin-bottom: 32px;">
        <div style="background: #f7fafc; padding: 16px; border-radius: 12px; border: 1px solid #e2e8f0;">
            <label style="color: #718096; margin-bottom: 4px; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em;">From</label>
            <p style="font-weight: 600; color: #2d3748; font-size: 16px;">{{ $message->full_name }}</p>
        </div>
        <div style="background: #f7fafc; padding: 16px; border-radius: 12px; border: 1px solid #e2e8f0;">
            <label style="color: #718096; margin-bottom: 4px; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em;">Email Address</label>
    ...
    <div style="background: #fff; padding: 24px; border-radius: 12px; border: 1px solid #e2e8f0; min-height: 200px;">
        <label style="color: #718096; margin-bottom: 12px; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em; display: block;">Message Body</label>
    ...
    <div style="margin-top: 32px; display: flex; justify-content: flex-end; gap: 12px;">
        <form method="post" action="{{ route('admin.messages.destroy', $message) }}">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this message forever?')">
